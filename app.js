...
const JIRA_PROXY = 'https://api.allorigins.win/raw?url=';
...
function toggleTokenVisibility() {
  const input = g('jira-api-token');
  const icon  = g('token-eye-icon');
...
async function connectJiraToken() {
  // Auto-clean URL — keep only the base domain
  let siteUrl = g('jira-site-url').value.trim();
  try {
    const parsed = new URL(siteUrl);
    siteUrl = parsed.protocol + '//' + parsed.host;
  } catch(e) {
    // not a valid URL, proceed and let validation catch it
  }
  siteUrl = siteUrl.replace(/\/$/, '');
  g('jira-site-url').value = siteUrl; // show cleaned URL to user

  const email   = g('jira-email').value.trim();
  const token   = g('jira-api-token').value.trim();

  g('jira-connect-error').style.display   = 'none';

  if (!siteUrl || !email || !token) {
    showConnectError('Please fill in all three fields before connecting.');
    return;
  }

  if (!siteUrl.includes('atlassian.net')) {
    showConnectError('Site URL should be in the format: https://yourcompany.atlassian.net');
    return;
  }

  g('jira-connect-loading').style.display = 'flex';

  try {
    // Test credentials by fetching the current user
    const authHeader = 'Basic ' + btoa(email + ':' + token);
    const testUrl    = `${JIRA_PROXY}${encodeURIComponent(siteUrl + '/rest/api/3/myself')}`;
    ...
    if (res.status === 401) throw new Error('Invalid email or API token. Please check and try again.');
    if (res.status === 403) throw new Error('Access denied. Make sure your API token has Jira read permissions.');
    if (!res.ok)            throw new Error(`Could not connect to Jira (${res.status}). Check your site URL.`);

    const userData = await res.json();

    // Store credentials in sessionStorage
    jiraCredentials = { siteUrl, email, token, authHeader };
    jiraSiteName    = userData.displayName + ' · ' + siteUrl;

    sessionStorage.setItem('jira_creds', JSON.stringify({ siteUrl, email, token }));
    ...
    g('jira-connect-loading').style.display = 'none';
  }
}

function showConnectError(msg) {
  const el = g('jira-connect-error');
...
function disconnectJira() {
  ...
  jiraSiteName    = null;
  sessionStorage.removeItem('jira_creds');
  ...
  // Clear form
  g('jira-api-token').value = '';
}

// ── Restore session ───────────────────────────────────────────
function restoreJiraSession() {
  const saved    = sessionStorage.getItem('jira_creds');
  const siteName = sessionStorage.getItem('jira_site_name');
  if (saved) {
    try {
      const { siteUrl, email, token } = JSON.parse(saved);
      jiraCredentials = {
        siteUrl, email, token,
...
async function fetchJiraStory() {
  ...
  if (!jiraCredentials) { showJiraError('Not connected to Jira. Please reconnect.'); return; }
  ...
  try {
    const apiUrl  = `${jiraCredentials.siteUrl}/rest/api/3/issue/${storyId}`;
    const proxyUrl = `${JIRA_PROXY}${encodeURIComponent(apiUrl)}`;

    const res = await fetch(proxyUrl, {
      headers: {
        'Authorization': jiraCredentials.authHeader,
        'Accept': 'application/json'
      }
    });

    if (res.status === 401) { disconnectJira(); throw new Error('Session expired. Please reconnect to Jira.'); }
    if (res.status === 404) throw new Error(`Story "${storyId}" not found. Check the ID and try again.`);
...
function estimateJiraCost() {
  ...
  const inputT      = (storyTokens + 20000 + 1500) * runs;
  ...
  g('jira-breakdown').innerHTML       =
    `Story content: <span>${fmtK(storyTokens)} tokens detected</span><br>` +
...
function renderJiraHistory() {
  ...
  g('jira-history-body').innerHTML = jiraHistory.map(h => {
    const cls = h.tag.includes('target') ? 'variance-good' : h.tag.includes('over') || h.tag.includes('Over') ? 'variance-over' : 'variance-warn';
    return `<tr>
      <td style="font-weight:500">${escHtml(h.id)}</td>
      <td class="muted">${h.est}</td>
      <td class="accent">${h.actual}</td>
      <td><span class="variance-tag ${cls}">${escHtml(h.variance)} ${escHtml(h.tag)}</span></td>
    </tr>`;
  }).join('');
}
